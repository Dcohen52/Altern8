...
class Parser:
    ...
    def func_def(self):
        res = ParseResult()
        if not self.curr_tok.matches(TT_KEYWORD, 'define'):
            return res.failure(InvalidSyntaxError(
                self.curr_tok.pos_start, self.curr_tok.pos_end,
                "Expected 'define' keyword"
            ))
        self.increment()
    ...
    def atom(self):
        ...
        if tok.type in (TT_INT, TT_FLOAT):
            ...
            include_def = res.register(self.func_include())
            if res.error: return res
            return res.success(include_def)
    ...
    def expr(self):
        ...
        if self.curr_tok.matches(TT_KEYWORD, 'var'):
            ...
            if self.curr_tok.type != TT_IDENTIFIER:
                return res.failure(
                    InvalidSyntaxError(self.curr_tok.pos_start, self.curr_tok.pos_end, "Expected Identifier"))
            var_name = self.curr_tok
            self.increment()
            if self.curr_tok.type != TT_EQUAL:
            ...
            if self.curr_tok.type == TT_AT:
                ...
                if self.curr_tok.type != TT_EQUAL:
                    return res.failure(
                        InvalidSyntaxError(self.curr_tok.pos_start, self.curr_tok.pos_end, "Expected '='"))
                self.increment()
                expr = res.register(self.expr())
                if res.error: return res
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(self.curr_tok.pos_start, self.curr_tok.pos_end,
                                   "Expected 'var', int, float, identifier, '+', '-', '(' or ')'"), self.register_count
            )
        return res.success(node)

    # INCLUDE
    def func_include(self):
        res = ParseResult()
        if self.curr_tok.matches(TT_KEYWORD, 'include'):
            self.increment()
            mn = self.curr_tok.value
            mn_in_path = f"modules/{mn}.alt8"
            try:
                if os.path.isfile(mn_in_path):
                    with open(mn_in_path, 'r') as f:
                        file_content = f.read()
                        ...
                        if len(alt8_functions) > 0:
                            print(f"Found {len(alt8_functions)} module functions in {mn_in_path}:")
                            module_namespace = {}
                            for function_name, args, function_code in alt8_functions:
                                ...
                                # print(f"Module Namespace: {module_namespace}\nend module_namespace")
                                print(f"    * {function_name}({args})")

                            return res.success(str(self.curr_tok.pos_end).strip(" "))
                        print("")

                else:
                    print(f"No module '{self.curr_tok.value}' in default modules folder.")
            except Exception as e:
                print(e)
            # return res.success(IncludeFunctionality(self.curr_tok.value))
        else:
            if self.curr_tok.type != TT_IDENTIFIER:
                return res.failure(InvalidSyntaxError(
                    self.curr_tok.pos_start, self.curr_tok.pos_end,
