...
class BuiltInFunction(BaseFunction):
    ...
    #         value = exec_ctx.symbol_table.get('path')
    #         print(str(value))
    #         return RTResult().success(String.null)
    #
    #     execute_include.arg_names = ['path']

    def execute_get_functions(self, exec_ctx):
        """
        Return a list of available functions in the chosen file from the default modules folder.
        :param exec_ctx: Module name
        :return: List of available functions
        """

        # TODO: Add support for inline functions

        fn = exec_ctx.symbol_table.get('fn')
        if not isinstance(fn, String):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "Module name argument must be string",
                exec_ctx
            ))

        # get_functions("example_2")

        fn = fn.value
        fn_in_path = f"modules/{fn}.alt8"
        try:
            if os.path.isfile(fn_in_path):
                with open(fn_in_path, 'r') as f:
                    file_content = f.read()
                    alt8_function_regex = r"define\s+(\w+)\s*\(([^)]*)\)\s*\n(.+?)\nend"
                    # r"define\s+(\w+)\s*\(([^)]*)\)\s*->\s*(.+?)$" - for inline functions
                    alt8_functions = re.findall(alt8_function_regex, file_content, re.DOTALL)

                    if len(alt8_functions) > 0:
                        print(f"Found {len(alt8_functions)} module functions in {fn_in_path}:")
                        module_namespace = {}
                        for function_name, args, function_code in alt8_functions:
                            full_function = f"\ndefine {function_name}({args})\n{function_code.strip()}\nend"
                            module_namespace[function_name] = full_function
                            exec_ctx.symbol_table.set(function_name, full_function)
                            print(f"    * {function_name}({args})")

                        # Make the module namespace available globally
                        # for name, code in module_namespace.items():
                        # globals()[name] = code
                        # exec_ctx.symbol_table.set(function_name, BuiltInFunction.print)
                        # BuiltInFunction.name = BuiltInFunction(name)
                        # exec_ctx.symbol_table(name, BuiltInFunction.print)

                        # exec_ctx.symbol_table.get('fn')
                        print("")
                        # print(f"\n{module_namespace}\n")
                        # print(full_function)
                        # print("\n- END include -\n")
            ...
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                f'\nFailed to load module \"{fn}\"\n"' + str(e),
                exec_ctx
            ))
        return RTResult().success(String.null)

    execute_get_functions.arg_names = ['fn']
...
BuiltInFunction.get_functions = BuiltInFunction("get_functions")
...
class Interpreter:
    ...
    def visit_ForNode(self, node, context):
        ...
        return res.success(
            String.null if node.return_null else List(elements).set_context(context).set_pos(node.pos_start,
                                                                                             node.pos_end)
        )
    ...
    # def visit_IncludeFunctionality(self, node, context):
    #     res = RTResult()
